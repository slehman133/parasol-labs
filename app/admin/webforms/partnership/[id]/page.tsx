...
const PartnershipWebformsPage = (props: { params: { id: string } }) => {
  ...
  const handleSendEmail = async () => {
    ...
    const error = await SendEmail({
      ...
      subject: "Parasol Laboratories: Engaging with Your Inquiry ",
      html: htmlMessage,
    ...
    if (!error) {
      ...
      form.status = Array.from(status).join(","); // Convert Selection to string
  ...
  const fetchForm = async () => {
    try {
      ...
      console.log(data);
      setForm(data);
      setStatus(new Set(data.status.split(",")));
    } catch (e) {
    ...
    <>
      <div className="flex flex-row gap-12 p-8 container">
        ...
        <div className="w-1/2">
          <div>
            <div>
              <h1 className="mx-auto text-center text-3xl font-bold my-4">
                Please Respond.
              </h1>
              <Divider />
              <div className="my-12">
